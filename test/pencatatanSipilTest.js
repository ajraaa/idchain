...
describe("PencatatanSipil", function () {
    ...
    it("warga dapat mengajukan permohonan", async () => {
        const tx = await pencatatan.connect(warga).submitPermohonan(0, "cid_json_xxx", 1, 0, 0, ethers.ZeroHash);
        await tx.wait();
    ...
    it("id permohonan harus bertambah satu per submit", async () => {
        await pencatatan.connect(warga).submitPermohonan(0, "cid_a", 1, 0, 0, ethers.ZeroHash);
        await pencatatan.connect(warga).submitPermohonan(1, "cid_b", 1, 0, 0, ethers.ZeroHash);
    ...
    it("mengembalikan status permohonan sebagai string", async () => {
        await pencatatan.connect(warga).submitPermohonan(0, "cid_test", 1, 0, 0, ethers.ZeroHash);
        const ids = await pencatatan.getPermohonanIDsByPemohon(warga.address);
    ...
    it("mengembalikan jenis permohonan sebagai string", async () => {
        await pencatatan.connect(warga).submitPermohonan(3, "cid_test", 1, 0, 0, ethers.ZeroHash); // 3 = Cerai
        const ids = await pencatatan.getPermohonanIDsByPemohon(warga.address);
    ...
    it("emit event saat verifikasi kalurahan", async () => {
        await pencatatan.connect(warga).submitPermohonan(1, "cid_kalurahan", 1, 0, 0, ethers.ZeroHash);
        const ids = await pencatatan.getPermohonanIDsByPemohon(warga.address);
    ...
    it("emit event saat verifikasi dukcapil", async () => {
        await pencatatan.connect(warga).submitPermohonan(2, "cid_dukcapil", 1, 0, 0, ethers.ZeroHash);
        const ids = await pencatatan.getPermohonanIDsByPemohon(warga.address);
    ...
    it("warga dapat membatalkan permohonan yang diajukan", async () => {
        await pencatatan.connect(warga).submitPermohonan(0, "cid_batal", 1, 0, 0, ethers.ZeroHash);
        const ids = await pencatatan.getPermohonanIDsByPemohon(warga.address);
    ...
    it("tidak bisa membatalkan permohonan yang bukan milik sendiri", async () => {
        await pencatatan.connect(warga).submitPermohonan(0, "cid_batal2", 1, 0, 0, ethers.ZeroHash);
        const ids = await pencatatan.getPermohonanIDsByPemohon(warga.address);
    ...
    it("kalurahan dapat mengambil daftar permohonan asalnya", async () => {
        await pencatatan.connect(warga).submitPermohonan(0, "cid_kalurahanasal", 1, 0, 0, ethers.ZeroHash);
    ...
    it("kalurahan dapat mengambil permohonan yang belum diverifikasi dengan status tertentu", async () => {
        await pencatatan.connect(warga).submitPermohonan(0, "cid_belumverif", 1, 0, 0, ethers.ZeroHash); // 0 = Diajukan
    ...
    it("dukcapil dapat mengambil permohonan dengan status tertentu", async () => {
        await pencatatan.connect(warga).submitPermohonan(0, "cid_for_dukcapil", 1, 0, 0, ethers.ZeroHash);
    ...
    it("dukcapil dapat verifikasi dengan dokumen resmi dalam satu transaksi", async () => {
        await pencatatan.connect(warga).submitPermohonan(0, "cid_for_dokres_combined", 1, 0, 0, ethers.ZeroHash);
        await pencatatan.connect(kalurahan).verifikasiKalurahan(0, true, "");
    ...
    it("gagal upload dokumen resmi jika sudah ada", async () => {
        await pencatatan.connect(warga).submitPermohonan(0, "cid_for_dokres_duplicate", 1, 0, 0, ethers.ZeroHash);
        await pencatatan.connect(kalurahan).verifikasiKalurahan(0, true, "");
    ...
    it("jumlahPermohonan bertambah setiap submit", async () => {
        ...
        await pencatatan.connect(warga).submitPermohonan(0, "cid_jumlah1", 1, 0, 0, ethers.ZeroHash);
        await pencatatan.connect(warga).submitPermohonan(1, "cid_jumlah2", 1, 0, 0, ethers.ZeroHash);
...
describe("Fitur Permohonan Pindah", function () {
    ...
    it("A1. Submit permohonan pindah berhasil", async () => {
        const tx = await pencatatan.connect(warga).submitPermohonan(4, "cid_pindah", 1, 2, 0, ethers.ZeroHash);
        await tx.wait();
        ...
        const perStatus = await pencatatan.connect(owner).getPermohonanForDukcapil(0); // Status.Diajukan
    ...
    it("B4. Verifikasi Kalurahan Asal berhasil (disetujui)", async () => {
        await pencatatan.connect(warga).submitPermohonan(4, "cid_pindah", 1, 2, 0, ethers.ZeroHash);
        const ids = await pencatatan.getPermohonanIDsByPemohon(warga.address);
...
describe("PencatatanSipil - onlyWargaTerdaftar", function () {
    ...
    it("berhasil submitPermohonan setelah registerWarga", async () => {
        const tx = await pencatatan.connect(warga).submitPermohonan(0, "cid_x", 1, 0, 0, ethers.ZeroHash);
        await tx.wait();
        const ids = await pencatatan.getPermohonanIDsByPemohon(warga.address);
        expect(ids.length).to.equal(1);
    });
    ...
    it("gagal batalkanPermohonan jika belum registerWarga", async () => {
        await pencatatan.connect(warga).submitPermohonan(0, "cid_x", 1, 0, 0, ethers.ZeroHash);
        const ids = await pencatatan.getPermohonanIDsByPemohon(warga.address);
